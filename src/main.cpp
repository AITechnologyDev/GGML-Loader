...
int main(int argc, char ** argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: %s <inspect|run|chat|bench|vae-decode|clip-encode|unet-denoise|txt2img> [args...]\n", argv[0]);
        return 1;
    }
    ...
    if (cmd == "unet-denoise")  return cmd_unet_denoise(argc - 1, argv + 1);
    if (cmd == "txt2img")       return cmd_txt2img(argc - 1, argv + 1);

    fprintf(stderr, "unknown subcommand '%s' (expected inspect|run|chat|bench|vae-decode|clip-encode|unet-denoise|txt2img)\n", cmd.c_str());
    return 1;
}
